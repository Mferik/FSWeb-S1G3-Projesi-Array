...
function dizi25Cesitmi(orijinalTatlar){
  return orijinalTatlar.length === 25 ? true : false
};
console.log(dizi25Cesitmi(orijinalTatlar)); // true
...
function sonCesitiKaldir(orijinalTatlar){
  orijinalTatlar.pop(orijinalTatlar.length -1)
  return orijinalTatlar
}
...
function ismeGoreCesitCikar(orijinalTatlar,cesit){
  const index= orijinalTatlar.indexOf(cesit);
  if (index !==-1) {
    orijinalTatlar.splice(index,1);
  }
  return orijinalTatlar;
}
console.log(ismeGoreCesitCikar(orijinalTatlar,"Nane"));
...
function ismeGoreFiltrele(orijinalTatlar, filterValue){
const filtreCesit=[];
for (let i=0; i< orijinalTatlar.length; i++){
  if (orijinalTatlar[i].includes(filterValue)){
    filtreCesit.push(orijinalTatlar[i]);
  }
}
return filtreCesit;
}
